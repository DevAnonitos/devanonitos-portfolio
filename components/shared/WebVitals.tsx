  ...
  return (
    <>
      ...
      <motion.button
        ...
        className='group bg-background/80 border-border/50 hover:bg-background fixed bottom-8 left-8 z-[100] flex items-center gap-3 rounded-full border p-2 pr-4 shadow-2xl backdrop-blur-md transition-all'
        ...
        {isOpen && (
          <div className='fixed inset-0 z-[110] flex items-center justify-center p-4'>
            ...
            <motion.div
              ...
              className='border-border/50 bg-card/80 shadow-3xl relative w-full max-w-lg rounded-[2.5rem] border p-8 backdrop-blur-2xl'
            >
              {/* Header */}
              <div className='mb-8 flex items-center justify-between'>
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className='hover:bg-muted/50 text-muted-foreground rounded-xl p-2 transition-colors'
                >
                  <X className='size-5' />
                </button>
              </div>
              ...
              <div className='grid grid-cols-2 gap-4'>
                {Object.entries(metrics).length > 0 ? (
                  Object.entries(metrics).map(([name, value]) => (
                    <div
                      key={name}
                      className='bg-background/40 border-border/40 space-y-3 rounded-3xl border p-5'
                    >
                  ...
                  <div className='col-span-2 space-y-4 py-12 text-center'>
                    <Activity className='text-muted/20 mx-auto size-12 animate-pulse' />
                    <p className='text-muted-foreground text-sm font-medium italic'>
                      Warming up engines... Just a second.
                    </p>
                  </div>
                )}
              </div>
