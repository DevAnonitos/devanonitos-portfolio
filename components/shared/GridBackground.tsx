...
const GridBackground = ({ className, variant = 'grid' }: GridBackgroundProps) => {
  return (
    <div className={cn('pointer-events-none absolute inset-0 -z-20 overflow-hidden', className)}>
      ...
      <div className='bg-background absolute inset-0 [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]' />
      ...
      {variant === 'dot' && (
        <div className='absolute inset-0 h-full w-full bg-[radial-gradient(#80808015_1px,transparent_1px)] [background-size:24px_24px]' />
      )}
      ...
      <div className='bg-primary/5 absolute top-0 left-0 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-[100px]' />
      <div className='absolute right-0 bottom-0 h-[500px] w-[500px] translate-x-1/2 translate-y-1/2 rounded-full bg-blue-500/5 blur-[100px]' />
    </div>
  )
}
